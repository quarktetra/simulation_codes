...
def sifter(layer,systems,p_shards,repair_time,failure_rate,DLtimes):   # feed this with the surviving systems. It will return the instances of data losses, and the systems that might still lose data
    ...
    for sysid in range(0,len(systems)):
        ...
        if layer==1:
            failuretimes=np.array(failuretimes  )
            #print(failuretimes)

        failuretimes=failuretimes[failuretimes<365.25*1]  # we are interested in the first year
        if layer==1:
            print("failures in the first year")
            print(failuretimes)
        sys_lost_data=0
...
def simulate(layer,failTimesFeed,total_shards, p_shards, afr, d_cap, r_speed,simulation_size_scale):
    DLtimes=[]
    d_shards =total_shards -p_shards
    repair_time=10**6*d_cap/r_speed/(60*60*24)
    failure_rate= -365.25/math.log(1-afr/100)     # this is in 1/days
    MTTDL_th=failure_rate*(failure_rate/repair_time)**p_shards /((d_shards)*factorial(total_shards)/(factorial(d_shards)*factorial(p_shards)));
    ...
    if layer==0:
        ...
        for sysid in range(0,sim_size):
            systems.append(np.random.exponential(failure_rate, total_shards))
        #print(systems[0])
    else:
        systems= failTimesFeed
        #print(systems[0])
    ...
    if p_shards>1:
        ...
        ptext=" parity"
    print("Please Wait: Running "+str(len(systems))+" simulations: "+str(total_shards) +" total shards with "+
        ...
        str(round(nines_th,2))+" nines. " + "; MTTDL_th_year="+str(round(MTTDL_th/365)))

    totalinstances=0
    while len(systems)>0:

        returned=sifter(layer,systems,p_shards,repair_time,failure_rate,DLtimes)
        systems=returned[0]
...
class Args:
  number_of_total_shards = 8
  number_of_parity_shards = 1
  annual_failure_rate_in_pct = 5
  drive_capacity_in_TB = 20
  recovery_speed_in_MBps=50
  simulation_size_scale=1
  Topnumber_of_total_shards = 8
  Topnumber_of_parity_shards = 1
  Toprecovery_speed_in_MBps=50
...
def main():
    ...
    parser.add_argument('Topnumber_of_total_shards', type=int),
    ...
    DLtimes=simulate(0,[], args.number_of_total_shards, args.number_of_parity_shards, args.annual_failure_rate_in_pct, args.drive_capacity_in_TB, args.recovery_speed_in_MBps,args.simulation_size_scale)
    failTimesFeed=[]
    splitedSize = args.Topnumber_of_total_shards
    failTimesFeed = [DLtimes[x:x+splitedSize] for x in range(0, len(DLtimes), splitedSize)]

    #print(failTimesFeed)
    print ("Number of bucket failures to feed to top:"+str( len(failTimesFeed))+ ". Bucketized to "+ str(args.Topnumber_of_total_shards))

    DLtimesT=simulate(1,failTimesFeed,args.number_of_total_shards, args.number_of_parity_shards, args.annual_failure_rate_in_pct, args.drive_capacity_in_TB, args.recovery_speed_in_MBps,args.simulation_size_scale)
    #print ("Number of data loss cases:"+str( len(DLtimesT)) )

    #np.savetxt('myfile.csv', DLtimes, delimiter=',')
    #print(DLtimes)
    #print(stats.weibull_min.fit(DLtimes,floc=0) )
