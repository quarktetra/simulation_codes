...
def sifter(systems,p_shards,repair_time,failure_rate,DLtimes):   # feed this with the surviving systems. It will return the instances of data losses, and the systems that might still lose data
...
def simulate(total_shards, p_shards, afr, d_cap, r_speed,simulation_size_scale):
    DLtimes=[]
    ...
    MTTDL_th=failure_rate*(failure_rate/repair_time)**p_shards /(factorial(total_shards)/(factorial(d_shards-1)));
    ...
    while len(systems)>0:
        ...
        returned=sifter(systems,p_shards,repair_time,failure_rate,DLtimes)
        systems=returned[0]
        totalinstances=totalinstances+returned[1]
    totalinstances=totalinstances*factorial(p_shards)      ## amending the simulation results     // repairs are not done in parallel.
   # print("Simulated "+str(sim_size)+ " systems with " +str(totalinstances ))
    ...
    return DLtimes,MTTDL_th/365;

class Args:
  number_of_total_shards = 19
  number_of_parity_shards = 1
  annual_failure_rate_in_pct = 5
  drive_capacity_in_TB = 20
  recovery_speed_in_MBps=50
  simulation_size_scale=2
  Topnumber_of_total_shards = 16
  Topnumber_of_parity_shards = 2
...
def main():
    ...
    simreturn=simulate( args.number_of_total_shards, args.number_of_parity_shards, args.annual_failure_rate_in_pct, args.drive_capacity_in_TB, args.recovery_speed_in_MBps,args.simulation_size_scale)
    DLtimes=simreturn[0]
    bottomMTTDLyear=  simreturn[1]
    bottomAFR_pct= 100/bottomMTTDLyear
    print ("Bottom layer AFR(%):"+str(bottomAFR_pct)+", We will simply recall the calculator with this AFR")
    simreturnT=simulate( args.Topnumber_of_total_shards, args.Topnumber_of_parity_shards, bottomAFR_pct, args.drive_capacity_in_TB, args.Toprecovery_speed_in_MBps,args.simulation_size_scale)
